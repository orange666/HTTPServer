...
public class ProcessorRequest  implements Runnable {
	...
	public ProcessorRequest(File directory ,String name ,Socket s){
		
		if(directory != null){
			this.rootDirectory = directory ;
		}
		if(name != null ){
		this.indexFileNname = name ;
		}
		this.connection = s ;
	}
	@Override
	public void run() {
		...
		try{
			...
			while(true){				
				char c = (char) in.read();   //如 GET/ HTTP/1.1
				if(c == '\r' || c == '\n' ||c == -1){
					break;
				}
	...
	private void doGet(DataInputStream in , OutputStream out, String reqLine) throws IOException{
		...
		String [] tokens = reqLine.split("\\s+");  //输入是报文的第一行 GET /HTTP/1.1
		String filePath = tokens[1];
		String fileName = filePath;
		if(filePath.indexOf('?') != -1){    //当GET请求带参时进行分析
		...
		String contentTpye = URLConnection.getFileNameMap().getContentTypeFor(fileName);
		String version = null ;
		if(tokens.length > 2){
			version = tokens[2];
		}
		...
		if(theFile.canRead() && theFile.getCanonicalPath().startsWith(rootDirectory.getPath())&& !theFile.isDirectory()){
